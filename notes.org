...
**** Binary search on safe points
** Progress
*** How to implement tydescs as addrspaces
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
**** Trans alloca's references to GC root that requires an addrspace
**** Make sure the drop interface takes a reference so that the language level interface matches what we're emitting, as in
...
**** Trans exports symbols for any tydescs which it gave an addrspace (e.g. _gc_tydesc_metadata_<modulename>_addrspace_<N>
**** LiveIRVariables needs to interpret pointer casts as the same value as the original pointer
**** Fake GC machine instructions need to be updated to take an addrspace
**** GC printer emits pointers to tydescs for any root with an addrspace > 1, using the naming convention above
**** GC metadata reader needs to be updated for new metadata format
**** Upsides
***** Trans does not need to emit gcroot intrinsics, this can happen later in automatic root insertion
**** Concerns
***** Non-word sized resources require references to be emitted in trans for rooting
***** Machine-level code has no concept of addrspaces, so we need to manually track them with our fake instructions
***** We're introducing language-specific logic in the GC printer (i.e. it has to know that addrspaces correspond to types)
*** How to implement tydescs as metadata
**** Trans manually emits gcroots with the tydesc pointer as the metadata
**** gcregroot needs to be extended take a metadata pointer
**** Fake GC machine instructions need to be extended to take a metadata pointer
**** GC printer emits metadata for each GC root
**** Upsides
***** Respects the intent of the existing gcroot infrastructure
**** Concerns
***** We're at the mercy of the optimizer to preserve metadata all the way through machine code
***** It's possible we'll be killing potential optimization opportunities by requiring metadata to be preserved
***** Potentially more work to do in SelectionDAG, et al
***** Patrick says gcregroot isn't preserved by LLVM IR optimizations
*** Chat with Patrick (pcwalton)
**** The reason why we can't do metadata is that gcregroot isn't actually intended to be a user-level intrinsic
**** The user doesn't add gcroot at all so that LLVM's IR optimization passes can freely optimize the roots
**** Then at the last possible moment (i.e. in automatic root insertion, which happens after LLVM's IR optimization passes are done), we add gcroot or gcregroot intrisics
**** These only serve the purpose of identifying roots through the machine-level passes so we can print them
***** So for example the machine optimizations might make additional copies of pointers in other registers that we don't know about
***** Which is why we can't use this system to build a copying GC
**** Thus in this scheme we really have no alternative to addrspaces, since we aren't emitting manual roots, and have to way to attach metadata to types
*** T_tydesc and tydesc_type is going to get awkward
**** Currently we generate the (singleton) tydesc LLVM type once at the start of trans
**** Since the drop glue is a part of the tydesc, the tydesc type includes the drop glue type
**** Thus if we want to customize the drop glue type for each resource type, we'll need to customize each tydesc type
**** But that means tydesc can't be a singleton any more
**** And everything becomes super awkward because we don't even have a crate context when creating the first tydesc type
**** And it's worse because there's other infrastructure that assumes glue_fn is a singleton type (i.e. T_glue_fn calls associate_type which assigns to a global map)
*** Chat with Patrick (pcwalton)
**** Cast the glue function at call time (i.e. call_tydesc_glue et al) instead of declaration time (i.e. lazily_emit_tydesc_glue et al)
**** That way you don't need to create multiple tydesc types, multiple glue types, etc
* 2012-08-02: Week 7 Thursday
** Adgenda
*** GC root for resources with addrspaces
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
**** Trans alloca's references to GC roots that requires an addrspace
**** Make sure the drop interface takes a reference so that the language level interface matches what we're emitting, as in
...
**** Binary search on safe points
** Progress
*** Added addrspace to tydesc_info so each type has its own tydesc, assigned by a generator in declare_tydesc
*** Cast the glue function in call_tydesc_glue_full so that it can take a pointer with the right addrspace
*** Looking for the place where we allocate resources so I can cast the addrspace
**** Not malloc_raw_dyn et al, since resources (at least the ones I care about) are on the stack
**** Maybe alloc_ty? (Calls alloca, so type is at least going on the stack)
**** Both of these refer to type_of to get the LLVM type. Neither seems to have logic specific to resources, nor can I find a caller who is specific to resources.
***** Problem with type_of is that it doesn't seem to access the tydesc, so it can't know which addrspace to stick things in (in the current scheme)
***** And, if we're allocating a resource on the stack, it's not like we're allocating a pointer anyway, so what would we cast?
***** Also, alloca is going to return a pointer in addrspace 0 no matter what we do; our first opportunity to cast the addrspace is after we get that pointer back
***** But maybe casting the alloca is the wrong thing to do, because if we e.g. alloca a pointer on the stack, and that pointer is in addrspace 1, we don't want the pointer to the pointer to also be in addrspace 1
**** Currently boxes are being put into addrspace 1 by T_box_ptr (called from type_of), then this is stripped away by non_gc_box_cast later
**** Resources are directly on the stack so there is no pointer; we need to alloca a pointer when we set up cleanup and set the address space there
***** I think I need to track down all the calls to add_root_cleanup
***** But add_root_cleanup doesn't seem to be capturing very many values (just judging by a print statement at the top of the file)
***** root_value => add_root_cleanup => add_clean_temp_mem => pushes drop_ty onto the stack
***** add_clean_temp_mem has a lot of call sites, so I'm hoping I can capture this some other way
****** And in some cases, add_clean_temp_mem is called with a type which doesn't need drop (e.g. bool)
****** Obviously I can't just assume the parameter to add_clean_temp_mem is a pointer
*** Summary:
**** What I think I'd like to is modify type_of so that everyone magically picks up the addrspace and everything just works
**** I'm not sure that can actually work
**** Specifically, there is no obvious 'root' type, at least as far as I can see
**** And also, it's not entirely trivial to get the addrspace inside type_of
**** Alternatively, you could modify calls sites to root_value, or add_root_cleanup, or add_clean_temp_mem, but then you have dozens of call sites to modify
* 2012-08-03: Week 7 Friday
** Adgenda
*** GC root for resources with addrspaces
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
...
**** Binary search on safe points
** Progress
*** Everything that goes through add_root_cleanup is already a box, and is already being put in addrspace 1
**** One of the other cleanup functions (add_clean_temp_mem and friends) must be responsible for resources
*** There are so many call sites to add_clean_* that I'd really rather fiddle with add_clean_* directly
*** Finished prototype of rooting for GC in add_clean_*, but it crashes at the beginning of stage3
*** It works on at least a single example (when I turn off rooting for add_clean_temp and add_clean_temp_mem)
* 2012-08-03: Week 7 Summary
** Finished LiveIRVariables pass (and fixed it for arguments and return values)
** Started working on per-type addrspaces; added addrspace to tydesc_info
** Got lost in trans for a day while figuring out where resource types were going
* 2012-08-06: Week 8 Monday
** Adgenda
*** GC root for resources with addrspaces
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
...
**** Binary search on safe points
** Progress
*** Debugging the segfault in stage3 compiler with resources rooted with addrspaces
**** Backtrace on the segfault in stage3

#0  0xb7a69edf in glue_free552 () from /home/elliottslaughter/rust/i686-unknown-linux-gnu/stage2/bin/../lib/libcore-c3ca5d77d81b46c1-0.3.so
#1  0xb7a69e49 in glue_drop551 () from /home/elliottslaughter/rust/i686-unknown-linux-gnu/stage2/bin/../lib/libcore-c3ca5d77d81b46c1-0.3.so
#2  0xb7a6a4fa in glue_drop564 () from /home/elliottslaughter/rust/i686-unknown-linux-gnu/stage2/bin/../lib/libcore-c3ca5d77d81b46c1-0.3.so
#3  0xb7a6a448 in glue_drop563 () from /home/elliottslaughter/rust/i686-unknown-linux-gnu/stage2/bin/../lib/libcore-c3ca5d77d81b46c1-0.3.so
#4  0xb7ae881c in __morestack () from /home/elliottslaughter/rust/i686-unknown-linux-gnu/stage2/bin/../lib/libcore-c3ca5d77d81b46c1-0.3.so
#5  0xb7a6a30b in task::private_methods::meth557::consume::_bc48ba59a88b8665::_03 () from /home/elliottslaughter/rust/i686-unknown-linux-gnu/stage2/bin/../lib/libcore-c3ca5d77d81b46c1-0.3.so
#6  0xb7a6a5e8 in task::task_builder::meth565::unlinked::_bc48ba59a88b8665::_03 () from /home/elliottslaughter/rust/i686-unknown-linux-gnu/stage2/bin/../lib/libcore-c3ca5d77d81b46c1-0.3.so
#7  0x0805f744 in task::try569::_38129365f51a37a::_00 ()
#8  0x0805e32a in monitor::_9712cd3b6261c0::_00 ()
#9  0x08068454 in __morestack ()
#10 0x08067f38 in main::_63bf702fc329a5d5::_00 ()
#11 0x08068018 in _rust_main ()
#12 0xb5c9d3a1 in task_start_wrapper (a=0x807c664) at /home/elliottslaughter/rust/src/rt/rust_task.cpp:159
#13 0x00000000 in ?? ()

**** The code for the call to glue_drop563

; <label>:57                                      ; preds = %50
  %58 = load { { i1, i1, %"enum option::option<comm::chan<task::notification>>[#0]", %"enum option::option<{mode: task::sched_mode,foreign_stack_size: option::option<u32>}>[#0]" }, { void ({ void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*, { void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }, %"enum option::option<util::noncopyable>[#0]", i1 } addrspace(5)** %5
  call void bitcast (void (i1*, i1*, %tydesc**, i8*)* @glue_drop563 to void (i1*, i1*, %tydesc**, { { i1, i1, %"enum option::option<comm::chan<task::notification>>[#0]", %"enum option::option<{mode: task::sched_mode,foreign_stack_size: option::option<u32>}>[#0]" }, { void ({ void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*, { void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }, %"enum option::option<util::noncopyable>[#0]", i1 } addrspace(5)*)*)(i1* null, i1* null, %tydesc** null, { { i1, i1, %"enum option::option<comm::chan<task::notification>>[#0]", %"enum option::option<{mode: task::sched_mode,foreign_stack_size: option::option<u32>}>[#0]" }, { void ({ void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*, { void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }, %"enum option::option<uti\
l::noncopyable>[#0]", i1 } addrspace(5)* %58)

**** The bitcast at the top glue_drop563

   %5 = bitcast i8* %3 to { { i1, i1, %"enum option::option<comm::chan<task::notification>>[#0]", %"enum option::option<{mode: task::sched_mode,foreign_stack_size: option::option<u32>}>[#0]" }, { void ({ void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*, { void (i1*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }*)*, { i32, %tydesc*, i8*, i8*, i8 } addrspace(1)* }, %"enum option::option<util::noncopyable>[#0]", i1 }*

**** No obvious problems with the emitted code, no great way to get debug info out of this, or even print values
*** Let's try separating out the patch for making glue functions take the correct types from the rest of the addrspace hacking
*** Not all glue functions take a double pointer
**** Some currently take a single pointer (notably, free)
**** So in addition to making sure that each kind of glue always takes a double pointer, you'll also have to change the contents of the glue to make sure they always treat that double pointer as a double pointer
***** Instead of casting it to a single pointer like they currently do
**** This is insane
**** Some free glue fns take the type itself (e.g. boxes, where the type already is a pointer)
**** Some take a pointer to the type (e.g. classes, where the class itself is obviously not a pointer)
**** I don't know if this is really worth my time, but it's hard to imagine making addrspaces work in any semi-sane way without fixing these issues first
* 2012-08-07: Week 8 Tuesday
** Adgenda
*** GC root for resources with addrspaces
**** Make all glue functions take explicit type parameters instead of bitcast everything
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
**** Trans alloca's references to GC roots that requires an addrspace
**** Make sure the drop interface takes a reference so that the language level interface matches what we're emitting, as in
...
**** Binary search on safe points
** Progress
*** I'm going to convert free to always take a T_ptr(type_of(t)) so that I don't end up with bizarre bitcasts in the glue functions
**** Start simple and push the correct types down the stack slowly
**** Created a free_ty_immediate which takes type_of(t) directly; free_ty now takes T_ptr(type_of(t))
**** Fixed a bug in type_of returning two different types for recursive enums
**** Seeing a pretty printer failure around 32-bit signed int boundaries, don't see how that could possibly be me, but checking anyway
**** Yup, shows up on incoming
**** Pushed, the other bug is someone else's problem
*** Branches
**** incoming <= fix-type_of-enums <= glue-fn-types
**** incoming <= better-liveness <= resource-addrspaces (also depends on glue-fn-types) <= gc
*** Fixed up static types for glue functions, which seem to be working now
*** Hopefully, this'll make it easier to root values with addrspace'd pointers now
*** Need to issue a pull request for https://github.com/elliottslaughter/rust/commits/glue-fn-types
* 2012-08-08: Week 8 Wednesday
** Adgenda
*** Issue a pull request for https://github.com/elliottslaughter/rust/commits/glue-fn-types
*** GC root for resources with addrspaces
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
**** Trans alloca's references to GC roots that requires an addrspace
...
**** Binary search on safe points
** Progress
*** Found one more bug in glue-fn-types (in tvec not sending the right type to cleanup)
*** Pushed glue-fn-types to incoming
*** Approach for addrspaces
**** I was going to push addrspaces all the way down the entire compiler using type_of_rooted, but I'm wondering if that's not a good strategy now
**** E.g. when iterating a struct where the elements require drop calls, now each GEP will need to be followed by a BitCast to convert from the structs's addrspace to the element's addrspace
**** Just seems like a huge mess, will touch a lot of code
**** Well, I can push this approach for a while to see how far it goes, and if I don't make progress come back tomorrow and try plan B (special case-ing add_clean_*)
* 2012-08-09: Week 8 Thursday
** Adgenda
*** GC root for resources with addrspaces
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
...
**** Binary search on safe points
** Progress
*** I realized that pushing addrspaces through glue fns will cause a lot of problems
**** Among other things those functions will now all pick up gcregroots, which I can't handle yet
**** But beyond that, I'm just touching a lot of code and it's not obvious that I need to be doing that much work
*** Back to addrspace-on-cleanup-only approach, although I saved a branch to my previous work (resource-addrspaces-glue)
*** Still seeing a crash when I try addrspaces-on-cleanup on add_clean_temp_mem
**** From a comment on add_root_cleanup:
...
**** Looks like add_clean_temp_mem might not be running in the context of the value being created, but rather in an outer scope
**** If that's the case, then it casts doubt on the entire add_clean_* approach
**** Not sure how to replace it though, even if I add my own cleanup layer, I'll need to intercept before the scope block is sent down
**** Currently doing a search on which add_clean_temp_mem calls are problematic (hopefully not all, but we'll see)
**** But either way I'll need a new abstraction, as even one "bad" call site will be a problem
* 2012-08-10: Week 8 Friday
** Adgenda
*** GC root for resources with addrspaces
**** Trans chooses some mapping between addrspace and tydesc (for all tydescs which are not boxes, but require destructors)
***** Do this when producing the drop function for each type, so that you can make the drop glue take a pointer in the right addrspace
...
**** Binary search on safe points
** Progress
*** Turns out *every single* call to add_clean_temp_mem causes crashes
**** I think I need a new abstraction
***** Somehow, I need to get the block context in which the root is valid
***** Or, I need the caller to pass me the root, instead of doing it automatically for them
**** Failure is happening because I'm not *revoking* the cleanup properly
***** i.e. because my fancy new cleanups are auto-rooting the value, they're not cleaning up the same value the caller thought they were cleaning up
***** So when the caller goes to revoke the cleanup, there is nothing to revoke
*** Proposal for new API for add_clean_*
**** root_for_cleanup -- roots a non-immediate value for cleanup
**** add_clean_root -- cleans up a non-temporary, rooted value
**** add_clean_temp_root -- cleans up a temporary, rooted value
**** add_clean_temp_immediate -- cleans up a temporary, non-immediate value
*** Mostly going well so far, small snag in trans_temp_lval
**** trans_temp_lval is sometimes called on non-immediates, but doesn't itself issue any revokations
**** Since literally every other call has been fixed by patching up the revokations, I'm guessing that's happening here
**** But if it is, it's somewhere deeper down the call stack; none of the direct callers revoke cleanups
**** According to the comment on top of trans_temp_lval, non-immediates shouldn't be going through there anyway, but I have no idea if that's up-to-date or not
**** Maybe I should factor trans_temp_lval to reject non-immediates?
**** Actually, no, that comment says 'non-intermediates' not 'non-immediates'
*** The problem is the value returned out of trans_temp_lval is being revoked elsewhere
**** Options?
***** Add root and rooted fields to lval_result
****** That's a lot of code to change
***** Add root_result which wraps lval_result (and you also need to pass some extra fields into move_val/copy_val)
****** That's still a lot of code to change
***** Always revoke the clean inside trans_temp_lval so that it can't escape
****** Nope, can't do that, we get memory leaks
**** Hacky, but I went with a lval_root to wrap lval_result
**** Touches a lot of code, not sure it's the right answer, but it appears to work
***** Darn, that's not all of them. This keeps snowballing into a bigger and bigger change
*** This is all wrong, the solution is in plain sight
**** The cleanup system already has a way of dealing with this
**** cleanups of variant cleanup_temp already have a 'val' field whose sole purpose is to match against revoke calls to make sure the right cleanup is revoked
**** That alone is sufficient to solve all my problems, and removes the need for all the code hacking I've been doing
**** Doh!
*** Looks like it's finally working
* 2012-08-10: Week 8 Summary
** Strict enforcement of glue fn types
*** Not really related to what I need to do, but at the time I thought that it would make my life easier
** Added addrspace rooting to all add_clean_* functions
** Split add_clean_temp into add_clean_temp_immediate and add_clean_temp_mem
* 2012-08-13: Week 9 Monday
** Adgenda
*** GC root for resources with addrspaces
**** Trans exports symbols for any tydescs which it gave an addrspace (e.g. _gc_tydesc_metadata_<modulename>_addrspace_<N>
**** LiveIRVariables needs to interpret pointer casts as the same value as the original pointer
***** Not really sure about this, since with optimization we might get a bitcast instead of an alloca
**** Fake GC machine instructions need to be updated to take an addrspace
**** GC printer emits pointers to tydescs for any root with an addrspace > 1, using the naming convention above
**** GC metadata reader needs to be updated for new metadata format
*** Everything else
...
**** Binary search on safe points
** Progress
*** Added emission of _gc_addrspace_metadata_* symbols in trans
*** Added emission of pointers to addrspace metadata in GenericGCPrinter
*** Somehow addrspaces aren't getting down to GenericGCPrinter (either that or Liveness isn't pickup up the root as live)
**** No, the code just wasn't expecting addrspaces > 1
**** The GC printer also wasn't expecting metadata to be a constant int
**** Fixed GC printer to handle these cases, now getting roots for my toy testcase
* 2012-08-14: Week 9 Tuesday
** Adgenda
*** GC root for resources with addrspaces
**** GC metadata reader needs to be updated for new metadata format
**** May need to turn off auto-rooting structs, because we now have explicit stack roots for them, and we don't want to double free
*** Everything else
**** Keep roots live long enough to outlive fail
**** Recurse on roots to get all live pointers
***** See https://github.com/mozilla/rust/issues/1498
...
**** Binary search on safe points
** Progress
*** Updated core::gc for new metadata format
*** Unexpectedly found that *tydesc is really **tydesc coming out of the root metadata
*** Now able to run drop for stack-allocated class instance from gc::cleanup_stack_for_failure()
*** Still some issues when calling malloc inside dtors
**** Was expecting an infinite loop, but got a segfault instead
**** Maybe the C wrapper isn't calling morestack properly?
**** Anyway, looks like a bad thing to do, generally
*** The GC cleaner seems to work for at least my personal case
*** Need to check the safety of the GC for when there is no stack map
**** Currently seeing crashes in std tests
* 2012-08-15: Week 9 Wednesday
** Adgenda
*** GC root for resources with addrspaces
**** Verify rooting on test suite
**** May need to turn off auto-rooting structs, because we now have explicit stack roots for them, and we don't want to double free
*** Everything else
**** Keep roots live long enough to outlive fail
**** Recurse on roots to get all live pointers
...
**** Binary search on safe points
** Progress
*** How to run GC tests so that the test runner uses GC *only* on tests and not on the various crate tests
**** Patch for the build system to turn on GC in tests

diff --git a/mk/tests.mk b/mk/tests.mk
index cf33cff..9040ea8 100644
--- a/mk/tests.mk
+++ b/mk/tests.mk
@@ -359,7 +359,7 @@ CTEST_COMMON_ARGS$(1)-T-$(2)-H-$(3) :=                                              \
...
**** Build script for configuring unoptimized, but building optimized
...
*** The tests in run-passes have a variety of failure modes
**** Some don't compile (gcregroot)
**** Some don't link (can't find _gc_module_metadata_*)
**** Some crash
**** Some infinite loop
*** Fixed a crash from tydesc being null
*** Tracking down linkage issue
**** Problem is library isn't creating it's own _gc_module_metadata_*
**** GenericGCPrinter isn't running on one particular lib
***** We aren't calling trans_closure anywhere in this lib
***** Maybe it's because the function takes a type parameter and we monomorphize function calls
* 2012-08-16: Week 9 Thursday
** Adgenda
*** GC root for resources with addrspaces
**** Verify rooting on test suite
**** May need to turn off auto-rooting structs, because we now have explicit stack roots for them, and we don't want to double free
...
**** Binary search on safe points
** Progress
*** Fixed linker error by not emitting GC metadata in crate map if we never call trans_closure
*** Verified that some cases are double freeing from auto-rooting structs
*** But now I'm getting some memory leaks (were those there before?)
*** Patrick mentioned that saving arguments to allocas should already happen (in e.g. copy_args_to_allocas or some such function)
*** Offending non-terminating test is run-pass/unwind-resource.rs
**** The problem is *not* an infinite loop, but that drop is never being called
**** This is the liveness after calling fail issue
**** Replacing fail with rt::rt_fail(ptr::null(), ptr::null(), 0) proves that that's the issue
**** trans_block_cleanups_ has a short-circuit clause for unreachability

if bcx.unreachable { return bcx; }

**** Probably good to turn that off, although I'm a little worried about emitting invalid code
**** Not sufficient, still not calling drop
**** Having a hard time pinning down where this goes through cleanup; debug trances look identical
* 2012-08-17: Week 9 Friday
** Adgenda
*** GC root for resources with addrspaces
**** Fix non-termination from not running dtors because roots aren't live after explicit fail
**** Check if this fixes the memory leaks, otherwise debug
**** Fix gcregroot emission if possible
**** Write a quick Python script to summarize the status of the tests
*** Everything else
**** Recurse on roots to get all live pointers
***** See https://github.com/mozilla/rust/issues/1498
**** Walk list of allocations to see what to free
...
**** Binary search on safe points
** Progress
*** Found that there is also an unreachable check in call_tydesc_glue_full
*** Fixed skipping cleanup due to unreachability
*** Tests don't get stuck now, but I'm crashing a lot in run-fail
*** Got me a Python script for summarizing failures
**** Only 4 tests are crashing or leaking at runtime in all of run-pass
**** But 62 crash or leak in run-fail
***** Still leaking memory on many tests
***** Still crashing (probably double free) on many tests
*** Looking at a double free
**** Definitely freeing the same pointer twice
**** Added a hash table to avoid calling free twice
**** Still broken?
**** Running tests again anyway to see if I fixed any other tests
**** Made 1 test in run-fail stop crashing
**** 61 to go
* 2012-08-17: Week 9 Summary
** Finished initial working resource cleanup in trans
** Fixed miscellaneous crashes
*** Fixed crash from null tydesc
*** Fixed linker error from module with no GC functions
*** Fixed double from auto-rooting structs
*** Fixed memory leaks resulting from bad liveness intervals (skipping dead cleanup code)
*** Fixed double free from two copies of the same pointer on the stack
** Wrote a Python script to summarize the state of the test suite
* 2012-08-20: Week 10 Monday
** Adgenda
*** GC root for resources with addrspaces
...
**** Binary search on safe points
** Progress
*** Roots might be null (i.e. unique pointers that have already been freed will be zeroed when I see them)
**** Adding the check for null causes 19 more tests to pass in run-fail
*** Still seeing lots of double frees (e.g. task-killjoin-rsrc)
**** Skipping for now, this testcase is really complicated
*** Leaking memory in unwind-unique.rs
**** I'm leaking memory because libcore isn't GC'd
**** Doh!
**** Can't fix this until I solve the gcregroot issue
*** Avoiding gcregroot
**** Trying special handling for args stored into allocas to see if I can avoid emitting gcregroot if possible
**** Might also be necessary for non-args
**** Alternatively, I could just turn off auto-root insertion for registers entirely
***** Obviously counter-productive to gettting GC working with optimization, but in the interest of getting it working non-optimized, it might have a shot at working
**** Looks like copy_args_to_allocas is not copying immediates to allocas
**** The obvious solution fixes 0 files in the test suite
*** Looks like gcregroot is coming from trans_estr in rustc/middle/trans/tvec.rs
**** Added a hack for this, fixed a couple of tests, but most are still failing
* 2012-08-21: Week 10 Tuesday
** Adgenda
*** GC root for resources with addrspaces
**** Fix various crashes and memory leaks
**** Fix gcregroot emission if possible
...
**** Binary search on safe points
** Progress
*** Trying to fix gcregroot emission by just turning it off entirely
**** Now rustc compiles itself with --gc -Z no-landing-pads
**** Test runner crashes though, so I'll still need some creative flag passing to get around that
**** With gcregroots off, but GC off in rustc, I'm passing another 40 tests or so
*** Might need to turn off GC inside core::gc because we malloc a hash table in the GC
**** Otherwise I might deallocate memory that the GC is actually using
**** Yup, fail is crashing
**** Fixed by adding a #[no_gc] attribute which turns off GC for a function
**** Wait, no, this was fixed by a print statement in cleanup_stack_for_failure
**** Wonder if issue is #[no_gc] does not extend to closures
**** Tried applying #[no_gc] at module level, but not clear how to do it
*** Alternative: Use task local memory to hold the root set
**** Tried @mut, but getting a double free
***** Maybe internal pointers are being rooted on the stack?
**** Tried moving allocation of @mut to another function, still get a double free
**** Tried wrapping @mut in unsafe pointer, get crash in deallocate
 
* 2012-08-22: Week 10 Wednesday
** Adgenda
*** GC root for resources with addrspaces
**** Fix various crashes and memory leaks
***** Notably, cleanup_stack_for_failure is crashing right now because it is collecting its own memory
*** Everything else
**** Recurse on roots to get all live pointers
***** See https://github.com/mozilla/rust/issues/1498
**** Walk list of allocations to see what to free
...
**** Binary search on safe points
** Progress
*** Ben Blum (bblum) directed me to unsafe::transmute and unsafe::bump_box_refcount as better ways to play with unsafe pointers
*** Fiddled with code, still crashing for all non-trivial testcases
*** Extracted unsafe rootset code into own module
**** It works: no crashes, and no double free (checked with valgrind)
**** Error must be something else in cleanup
*** Was missing a check refcount == -1 on branch for exchange heap
**** But it doesn't actually make any tests pass that didn't previously
*** Question: Does the box annihilator call drop glue or free glue? Did it get updated when I changed free glue to always take aliases?
* 2012-08-23: Week 10 Thursday
** Adgenda
*** GC root for resources with addrspaces
**** Fix various crashes and memory leaks
***** Notably, cleanup_stack_for_failure is crashing right now because it is collecting its own memory
...
**** Binary search on safe points
** Progress
*** Actually, I'm crashing in walk_gc_roots::anon even when I'm doing a dry run
**** The problem isn't the hash table, because I've commented it out
**** The problem isn't the boxes in the testcase, because I'm not even reaching them
**** I think the problem might be stack closures used in stack walking might be recognized as boxed closures
**** Trying to skip everything above the caller to cleanup_stack_for_failure so I don't have to worry about any of this
**** If this works, make sure to check deallocating closures of various types
**** Doesn't crash any more, but still leaking
**** Better approach: Leave a sentinel value on the stack and then skip stack frames until you get to it
**** That way you don't have to care that much about inlining
*** The alloc 4 ~int's and fail testcase is leaking
**** We're printing the stack frame for the 4 ~int's
**** But we don't find the sentinel first, so we skip past them
**** Maybe we're not recongizing the safe point in cleanup_stack_for_failure correctly?
**** No, it's not complaining about not having safe point info
**** This particular safe point must just not have any entries in it
**** Tried adding a print statement for the sentinel at the end of the function
***** Now I find the sentinel again, but also crash
*** Outstanding
**** safe points for 4 ~int's testcase
**** closures (stack or otherwise)
**** sentinels XOR no GC in libcore?
***** Maybe use a build-time flag to be compatible with non-GC builds
* 2012-08-24: Week 10 Friday
** Adgenda
*** GC root for resources with addrspaces
**** Fix various crashes and memory leaks
***** Notably, I'm missing the sentinel value in some cases (based on liveness?)
**** Write email with plan of attack for committing GC
**** Questions to answer
***** safe points for 4 ~int's testcase
***** closures (stack or otherwise)
***** sentinels XOR no GC in libcore?
...
**** Binary search on safe points
** Progress
*** Plan for sentinels when GC is off in libcore
**** Add #[cfg(gc)] and #[cfg(nogc)] options to the compiler
**** Write a function which tells the GC whether to expect a sentinel value or not, conditional on compiling with GC
*** Pushed resource-addrspaces to mozilla/incoming
*** After rebasing, seeing linking failures due to missing tydescs
**** Looks like gen_shape_tables is enumerating some types which hadn't been used previously
**** Since gen_shape_tables was called after emit_tydescs, those tydescs weren't being emitted into the file
**** Swapped the call order of gen_shape_tables and emit_tydescs
**** Not sure if that will have any other repercussions
**** Command line processing seems to be completely broken
***** How that's related, I have no idea
**** Call graph
***** gen_shape_tables => gen_resource_shapes => gen_res_dtor => monomorphize_fn => ... => get_tydesc
**** Best guess is that emit_tydescs is also somehow tied into gen_shape_tables (i.e. it's a circular dependency)
**** Trying to see if I can break that dependency by trans-ing all the resource dtors first
**** Patch works in isolation, now to see if it actually fixed the GC build
* 2012-08-24: Week 10 Summary
** Fixed freeing null pointers (from already-deallocated unique pointers)
** Turned of GC register roots temporarily to get unoptimized build working
** Added sentinel hack to avoid collecting memory in use by the GC
** Pushed resource-addrspaces branch to incoming
* 2012-08-27: Week 11 Monday
** Adgenda
*** GC root for resources with addrspaces
**** Fix linker error from rebase
**** Fix various crashes and memory leaks
***** Notably, I'm missing the sentinel value in some cases (based on liveness?)
**** Fix for GC being off in libcore by adding #[cfg(gc)] and #[cfg(nogc)] options to the compiler
**** Write email with plan of attack for committing GC
**** Questions to answer
***** safe points for 4 ~int's testcase
***** closures (stack or otherwise)
...
**** Binary search on safe points
** Progress
*** The linker error from rebase is specifically from *turning off landing pads*
**** Now I can finally repro on my fix-missing-tydescs branch (and incoming)
**** Turns out I need to call shape_of in force_declare_tydescs to fully break the cycle
**** Patch is now in incoming, waiting for snapshot
**** Also, GC branch (when rebased back to the point pre-snapshot) builds itself again
***** But it has issues with running the test suite because we freeze in the testcase that depends on dtors running
*** Tracking down leakage of sentinel value in GC
**** I wonder if this is really a liveness issue, because a trivial keepalive at the end of GC doesn't fix it
***** Not sure LLVM is smart enough for that anyway
**** Actually, now I'm losing the sentinel even in the fail case (didn't I used to get it?)
**** LLVM is definitely spitting out an identical list of stack offsets for each safe point
**** The address within cleanup_stack_for_failure seems to be shifting around between fail.rs and leak.rs
***** Not really sure what the cause is, as there really ought to only be one safe point of relevance in the function
***** Trying to push sentinel down to rt::rt_fail to see if that stabilizes the function
***** Now it looks like I can always find the sentinel
***** Still leaking from pthread_create
*** Also, the core-without-GC build is also broken right now, because it's looking for the sentinel
* 2012-08-28: Week 11 Tuesday
** Adgenda
*** GC root for resources with addrspaces
**** Fix for GC being off in libcore by adding #[cfg(gc)] and #[cfg(nogc)] options to the compiler
**** Write email/blog post describing the status of GC in rust
**** Questions to answer
***** safe points for 4 ~int's testcase
***** closures (stack or otherwise)
***** sentinels XOR no GC in libcore?
...
**** Binary search on safe points
** Progress
*** Coded up #[cfg(gc)] and conditional sentinel check
**** Problem is it won't compile without another snapshot
* 2012-08-29: Week 11 Wednesday
** Adgenda
*** Work on talk
** Progress
*** Worked on talk
*** Created github repo https://github.com/elliottslaughter/rust-gc-talk
* 2012-08-30: Week 11 Thursday
** Adgenda
*** Give talk
*** GC root for resources with addrspaces
**** Verify GC when GC is off in libcore
**** Leak pthread handle in 4 ~int's case?
**** Write email/blog post describing the status of GC in rust
**** Questions to answer
***** Stack closures refcount field uninitialized?
*** Everything else
**** Recurse on roots to get all live pointers
***** See https://github.com/mozilla/rust/issues/1498
**** Walk list of allocations to see what to free
...
** Progress
*** Gave talk
*** Verified that the GC once again works with GC off in libcore, after rebasing with snapshot
**** We're failing one extra run-fail test, but otherwise test results look exactly the same as before the rebase
*** The leak in 4 ~int's isn't from pthreads, it's an assert in memory_region that's causing us to exit early
**** It's asserting on live_allocations < 1, *not* >= 1
**** With RUSTRT_TRACK_ALLOCATIONS set to 3 in rt/memory_region.h, it claims the pointers don't refer to allocations
**** But these pointers are the same ones I printed out in the debug statements at the top of main
**** Maybe double free?
**** But there's no explicit free in the LLVM assembly code before the call to fail
* 2012-08-31: Week 11 Friday
** Adgenda
*** GC root for resources with addrspaces
**** Still leaking in 4 ~int's case?
**** Write email/blog post describing the status of GC in rust
**** Questions to answer
***** Stack closures refcount field uninitialized?
*** Conservative GC (if you have time)
*** Everything else
**** Recurse on roots to get all live pointers
***** See https://github.com/mozilla/rust/issues/1498
**** Walk list of allocations to see what to free
...
**** Binary search on safe points
** Progress
*** Looks like the box annihilator didn't actually know how to deallocate unique boxes
**** Was trying to deallocate everything out of the task-local heap
**** Yeah, that doesn't work so well
**** Testing fix
**** Fixes 5 tests in the without-GC-in-libcore build, but the with-GC-in-libcore build is still getting stuck in tests
**** Notably, unwind-unique is still leaking
*** Leaks when unwinding task
**** Not finding sentinel
**** Guess my last fix didn't work as well as I thought it did
**** Look at these stack offsets

                                                        /==============================\
sentinel: 0xb69067a8 <==================================| Here's the original pointer. |
rootset: root 0xb66012f4 box 0xb6602d58                 \==============================/
stack frame: fp 0xb6601118 pc 0x00000000
stack frame: fp 0xb6601138 pc 0xb7579ac2
stack frame: fp 0xb66011b8 pc 0xb7521d1f
  root 0xb6601190 (fp 0xb66011b8 offset -40)
    skipping (not yet sentinel): root 0xb6601190
  root 0xb6601194 (fp 0xb66011b8 offset -36)
    skipping (not yet sentinel): root 0xb6601194
  root 0xb66011a0 (fp 0xb66011b8 offset -24)
    skipping (not yet sentinel): root 0xb66011a0
  root 0xb66011a8 (fp 0xb66011b8 offset -16)
    skipping (not yet sentinel): root 0xb66011a8
stack frame: fp 0xb6601268 pc 0xb7521b35
stack frame: fp 0xb6601308 pc 0xb75238a1
  root 0xb66012f4 (fp 0xb6601308 offset -20)
    skipping (not yet sentinel): root 0xb66012f4
stack frame: fp 0xb690673c pc 0xb757a740
  is_safe_point: no safe point for pc 0xb757a740
stack frame: fp 0xb69067b8 pc 0xb75236c8                /================================================================================================\
  root 0xb69067a4 (fp 0xb69067b8 offset -20) <==========| This is off by only 4 bytes. If the offset was -16 instead of -20, then this would be correct. |
    skipping (not yet sentinel): root 0xb69067a4        \================================================================================================/
stack frame: fp 0xb69067f8 pc 0x08048a84
  root 0xb69067e8 (fp 0xb69067f8 offset -16)
    skipping (not yet sentinel): root 0xb69067e8
stack frame: fp 0xb6906938 pc 0xb751b707
  root 0xb690688c (fp 0xb6906938 offset -172)
    skipping (not yet sentinel): root 0xb690688c
  root 0xb69068ac (fp 0xb6906938 offset -140)
    skipping (not yet sentinel): root 0xb69068ac
  root 0xb69068bc (fp 0xb6906938 offset -124)
    skipping (not yet sentinel): root 0xb69068bc
  root 0xb69068cc (fp 0xb6906938 offset -108)
    skipping (not yet sentinel): root 0xb69068cc
  root 0xb69068dc (fp 0xb6906938 offset -92)
    skipping (not yet sentinel): root 0xb69068dc
  root 0xb69068e4 (fp 0xb6906938 offset -84)
    skipping (not yet sentinel): root 0xb69068e4
  root 0xb6906904 (fp 0xb6906938 offset -52)
    skipping (not yet sentinel): root 0xb6906904
ERROR: never found sentinel!!!

**** Maybe the stack offsets are getting messed up somehow (e.g. this one should have been -16 and somehow it became -20)
**** In working examples, the offset is indeed -16 instead of -20
**** But we're linking against the same libcore, so how is this possible?
*** More about strange offsets in libcore
**** There are actually two roots in rt_fail
* 2012-08-31: Week 11 Summary
** Fixed linker error from not declaring all tydescs before emit_tydescs
** Fixed build without GC enabled in libcore
** Gave talk on Rust GC
** Pushed sentinel value further down the stack in an attempt to fix issues finding sentinel values
** Verified that I am in fact getting bogus offsets
* 2012-09-04: Week 12 Tuesday
** Adgenda
*** GC root for resources with addrspaces
**** Fix missing sentinel from incorrect offsets
**** Write email/blog post describing the status of GC in rust
**** Questions to answer
***** Stack closures refcount field uninitialized?
*** Try Conservative GC (if you have time)
*** Everything else
**** Recurse on roots to get all live pointers
***** See https://github.com/mozilla/rust/issues/1498
**** Walk list of allocations to see what to free
...
**** Binary search on safe points
** Progress
*** Debugging missing sentinel
**** Added embedded function names in function metadata
**** Traces look really strange--sometimes missing functions, sometimes have duplicate entries for same function
**** I see strangeness on all of my sanity tests--even ones which appear to be working correctly (the strangeness just happens up the stack, so it doesn't matter)
***** E.g. the 4 ~int's testcase, which I "fixed" by moving the sentinel down into rt::rt_fail, is missing a stack frame for gc::cleanup_stack_on_failure
**** In every example, I see the following pattern in the stack frames
***** function_1
***** anonymous (no safe point entry)
***** function_1 (expected function_2)
**** Is the middle stack frame __morestack?
**** What happens with return addresses around __morestack boundaries?
*** Chat with Brian (brson)
**** Create a rust_builtin function (ok to run on C stack) to return rust_task::stk
***** This gives us the end of a doubly linked list of stack segments (i.e. containing the most recent stack frame)
***** Chasing the prev pointers of the stk_seg structs should provide earlier segments
***** The bounds of each stk_seg can be obtained from the pointer to the stk_seg itself (lower bound) and the stk_seg::end pointer (upper bound)
**** When walking the stack, for each frame
***** Check if it is contained in the current stack segment
***** If yes, proceed as usual
***** If not, check if it is contained in the prev stack segment (and prev != original, i.e. we haven't wrapped around)
****** If yes, then we're straddling a stack boundary, and make sure to use fp+8 as the return address rather than fp+4
****** If not, skip this frame (since we're somewhere above the segment we originally started at)
* 2012-09-05: Week 12 Wednesday
** Adgenda
*** GC root for resources with addrspaces
**** Fix finding safe points on segmented stack
**** Write email/blog post describing the status of GC in rust
**** Questions to answer
***** Stack closures refcount field uninitialized?
*** Try Conservative GC (if you have time)
...
**** Binary search on safe points
** Progress
*** Wrote patch for fixing segmented stack
**** Works on my sanity checks
**** Running tests now
**** Doesn't seem to have any affect on without-GC-in-libcore build
**** Build with-GC-in-libcore is still hanging
* 2012-09-06: Week 12 Thursday
** Adgenda
*** GC root for resources with addrspaces
**** Write email/blog post describing the status of GC in rust
**** Questions to answer
...
**** Binary search on safe points
** Progress
*** Failures on 64-bit Linux
**** coretest vec::tests::test_any2_and_all2
**** Spinning up 64-bit Linux server to investigate issues there
**** Attempted patch to avoid uninitialized reads in 64-bit Linux
***** Theory was that the return address in the morestack case might depend on the word size
***** Nope, still fails
*** Failures on 32-bit Windows
**** test\run-pass\anon-extern-mod-cross-crate-2.rs
**** stdout and stderr are blank
* 2012-09-07: Week 12 Friday
** Adgenda
...
**** Binary search on safe points
** Progress
*** Added early escape to GC in case GC isn't enabled (fixes 64-bit Linux)
*** Pushed GC branch to incoming
*** Testing workaround for Windows to fix presumed crash there
**** Never mind, I just realized that this test was failing before GC
**** Apparently the buildbot for incoming doesn't actually run the entire test suite
*** Possibly spurious crash on FreeBSD?
**** Didn't see this before the rebase, not sure how rebase could have interacted with GC to cause this
***** But this passed on the commit right before mine, so it looks like my problem, unless it's a spurious crash
**** src/test/bench/task-perf-spawnalot.rs
**** FreeBSD passed on the commit right after mine, so I think this was a fluke
*** All tests now passing (with GC off)
